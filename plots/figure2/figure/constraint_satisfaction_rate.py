...
bosonic_by_layer = bosonic_df.groupby(['num_vars','num_cons'])['incons_rate'].apply(np.mean).to_dict()
for key in bosonic_by_layer.keys():
    numvars,numcons = key
    if numvars in avg_sat_cv.keys():
        avg_sat_cv[numvars].update({numcons:bosonic_by_layer[key]})
    else:
        avg_sat_cv[numvars]= {numcons:bosonic_by_layer[key]}

avg_sat_cv['avg'] = bosonic_df.groupby(['num_cons'])['incons_rate'].apply(np.mean).to_dict()
print(avg_sat_cv)

# 按变量数和约束数组织数据（约束满足率）- 从qaoa数据源
for var in variables:
    avg_sat_ours[var] = {}
    for cons in constraints:
        subset = df_qaoa[(df_qaoa['变量数'] == var) & (df_qaoa['约束数'] == cons)]
        if not subset.empty:
            avg_sat_ours[var][cons] = subset['约束满足率'].mean()

# 按变量数和约束数组织数据（约束满足率）- 从chocoq数据源
for var in variables:
    avg_sat_dv[var] = {}
    for cons in constraints:
        subset = df_chocoq[(df_chocoq['变量数'] == var) & (df_chocoq['约束数'] == cons)]
        if not subset.empty:
            avg_sat_dv[var][cons] = subset['约束满足率'].mean()
...
for cons in constraints:
    ...
    if not subset.empty:
        avg_sat_ours['avg'][cons] = subset['约束满足率'].mean()
...
for cons in constraints:
    ...
    if not subset.empty:
        avg_sat_dv['avg'][cons] = subset['约束满足率'].mean()
...
# 创建双栏图表
def create_constraint_satisfaction_chart(sat_ours, sat_dv,sat_cv, title, ylabel, filename):
    ...
    for var in all_variables:
        if var in sat_ours:
            var_constraints[var] = sorted(sat_ours[var].keys())
        else:
            var_constraints[var] = []
    ...
    for i, cons in enumerate(all_constraints):
        ...
        for var in all_variables:
            ...
            if (var in sat_ours and cons in sat_ours[var] and
                var in sat_dv and cons in sat_dv[var]):
                values_qaoa.append(sat_ours[var][cons])
            else:
        ...
        for var in all_variables:
            ...
            if (var in sat_ours and cons in sat_ours[var] and
                var in sat_dv and cons in sat_dv[var]):
                values_chocoq.append(sat_dv[var][cons])
            else:
                values_chocoq.append(np.nan)  # 使用NaN表示缺失数据
        values_cv = []
        for var in all_variables:
            # 只有当两个数据源都存在这个变量数和约束数组合时才显示
            if (var in sat_cv and cons in sat_cv[var]):
                values_cv.append(sat_cv[var][cons])
            else:
    ...
    for i, var in enumerate(all_variables):
        ...
        for j, cons in enumerate(var_constraints[var]):
            ...
            if idx >= 0:
                ...
                # 添加约束数标签
                ax1.text(label_x, ax1.get_ylim()[0] - (ax1.get_ylim()[1] - ax1.get_ylim()[0]) * 0.05, 
    ...
    for i, var in enumerate(all_variables):
        if var != 2:  # 排除4变量
            for cons in all_constraints:
                ...
                if (var in sat_ours and cons in sat_ours[var] and
                    var in sat_dv and cons in sat_dv[var]):
                    qaoa_value = sat_ours[var][cons]
                    chocoq_value = sat_dv[var][cons]
                    
                    # 计算比率（qaoa/chocoq）
                    if chocoq_value != 0:
                        ratio = qaoa_value / chocoq_value
    ...
    # 设置右侧Y轴（比率）
    ax2.set_ylabel('Ratio (qaoa/chocoq)', color='red')
    ...
    if h1 and h2:
        ...
        legend_elements.append(
            plt.Line2D([0], [0], color='red', 
                      marker='o', markersize=8, label='qaoa/chocoq ratio')
        )
...
# 创建约束满足率图表
create_constraint_satisfaction_chart(avg_sat_ours, avg_sat_dv,avg_sat_cv, 'Constraint Satisfaction Rate Comparison', 'Constraint Satisfaction Rate', 'constraint_satisfaction_rate')
